...
class tsp_input{
    ...
    tsp_input(){
        ...
        dis=new int[n][n];
        start=sc.nextInt();
        for(int k=0,i,j,d=j=i=0;k<(n*(n-1))/2;k++) {
            i=sc.nextInt();j=sc.nextInt();d=sc.nextInt();
            dis[i][j]=dis[j][i]=d;
        }
        for(int i=0;i<n;i++) for(int j=0;j<n;j++) if(dis[i][j]<1 && i!=j) {
            System.err.println("INVALID "+i+" "+j+" "+dis[i][j]);
            System.exit(-1);
        }
