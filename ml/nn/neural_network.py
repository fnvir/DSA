...
class NeuralNetwork:
    def __init__(self,shape:tuple,cost_func:Activation):
        self.cost_func=cost_func
        self.network=[]
        for i in range(1,len(shape)):
            self.network.append(Layer(shape[i-1],shape[i],tanh))
    ...
    def _backprop(self,output_grad,alpha):
        for i in range(len(self.network)-1,-1,-1):
            output_grad=self.network[i].backward(output_grad,alpha)

    def gradient_descent_stochastic(self,x_train,y_train,batch_size=None,learning_rate=0.01,max_iter=1000,epsilon=1e-20,show=False):
        m=len(x_train)
        ...
        for e in range(max_iter):
            ...
            for i in z:
                ...
                self._backprop(output_grad,learning_rate)
                # cost
